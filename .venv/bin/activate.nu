...
export-env {
    ...
    let virtual_env = r#'/Users/ritkumar17/Desktop/RD_Project/data_science_interview/.venv'#
    let bin = r#'bin'#
    let path_name = if (has-env 'Path') { 'Path' } else { 'PATH' }
    let venv_path = ([$virtual_env $bin] | path join)
    let new_path = ($env | get $path_name | prepend $venv_path)
    let virtual_env_prompt = if (r#'data-science-interview-py3.12'# | is-empty) {
        ($virtual_env | path basename)
    } else {
        r#'data-science-interview-py3.12'#
    }
